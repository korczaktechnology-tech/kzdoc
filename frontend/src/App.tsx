...
function DocumentTable({docs,onSelect,onAction}:{docs:DocumentItem[];onSelect:(d:DocumentItem)=>void;onAction:(d:DocumentItem)=>void}){
...
function App(){
  ...
  const[folders,setFolders]=useState<Folder[]>([]),[users,setUsers]=useState<User[]>([]),[groups,setGroups]=useState<Group[]>([]),[events,setEvents]=useState<Event[]>([]),[notes,setNotes]=useState<Notification[]>([]);
  ...
  async function load(v:View=view){try{setError('');
    ...
    if(v==='folders')setFolders(await api.folders()); if(v==='users'||v==='admin')setUsers(await api.users()); if(v==='groups')setGroups(await api.groups());
