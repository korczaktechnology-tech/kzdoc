...
function App(){
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      {(user.role==='admin'||user.role==='manager')&&<><div className="nav-label">ADMINISTRAÇÃO</div><nav className="side-nav">{user.role==='admin'&&<button className={'nav-item '+(view==='users'?'active':'')} onClick={()=>setView('users')}><Icon name="user"/><span>Usuários</span></button>}<button className={'nav-item '+(view==='groups'?'active':'')} onClick={()=>setView('groups')}><Icon name="users"/><span>Grupos</span></button></nav></>}
