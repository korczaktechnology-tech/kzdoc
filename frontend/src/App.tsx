...
function Home({user,docs,notes,onNew,onSelect,onAction,setView}:{user:User;docs:DocumentItem[];notes:Notification[];onNew:()=>void;onSelect:(d:DocumentItem)=>void;onAction:(d:DocumentItem)=>void;setView:(v:View)=>void}){
  ...
  return <>
    ...
    <div className="quick-actions">
      <button onClick={onNew}><span className="quick-icon blue">▤</span><div><strong>Novo documento</strong><small>Criar um novo arquivo</small></div><b>→</b></button>
      <button onClick={()=>setView('folders')}><span className="quick-icon blue">□</span><div><strong>Nova pasta</strong><small>Organizar seus arquivos</small></div><b>→</b></button>
      <button onClick={onNew}><span className="quick-icon purple">↥</span><div><strong>Fazer upload</strong><small>Enviar arquivos do seu dispositivo</small></div><b>→</b></button>
      <button onClick={()=>setView('documents')}><span className="quick-icon gray">⌗</span><div><strong>Digitalizar</strong><small>Converter documentos em PDF</small></div><b>→</b></button>
    </div>
    <div className="home-columns">
      <Section title="Seus arquivos" actions={<div className="view-switch"><button className="active">☷</button><button>⊞</button></div>}><div className="file-tabs"><button className="active">Todos</button><button>Documentos</button><button>Imagens</button><button>Vídeos</button><button>Outros</button></div><DocumentTable docs={docs.slice(0,8)} onSelect={onSelect} onAction={onAction}/><button className="see-all" onClick={()=>setView('documents')}>Ver todos os documentos <span>→</span></button></Section>
      <div className="home-side">
        <section className="side-card storage"><div className="card-title"><h2>Armazenamento</h2><button>♧</button></div><div className="storage-ring"><div><strong>{Math.min(99,Math.round((docs.length/Math.max(docs.length+8,1))*100))}%</strong></div></div><div className="storage-copy"><strong>{docs.length} documentos</strong><span>de 100 GB</span></div><div className="progress"><span style={{width:Math.min(95,Math.max(8,docs.length*8))+'%'}}/></div><button className="manage-button">Gerenciar armazenamento <span>→</span></button></section>
        ...
        <section className="side-card"><div className="card-title"><h2>Links rápidos</h2></div><button className="link-row" onClick={()=>setView('folders')}>♧ <span>Pastas compartilhadas</span><b>›</b></button><button className="link-row" onClick={()=>setView('favorites')}>☆ <span>Documentos favoritos</span><b>›</b></button><button className="link-row" onClick={()=>setView('documents')}>⊞ <span>Projetos em andamento</span><b>›</b></button><button className="link-row" onClick={()=>setView('trash')}>⌫ <span>Lixeira</span><b>›</b></button></section>
      </div>
    </div>
    <div className="dashboard-foot"><span>♢ Seus dados estão protegidos com criptografia de ponta a ponta.</span><span>Korczak Technologies&nbsp; • &nbsp;Korczak Documents v1.0.0</span></div>
  </>
}
...
function App(){
  ...
  const[folders,setFolders]=useState<Folder[]>([]),[deletedFolders,setDeletedFolders]=useState<Folder[]>([]),[tags,setTags]=useState<{id:string;owner_id:string;name:string}[]>([]),[moveModal,setMoveModal]=useState(false),[showFolderTrash,setShowFolderTrash]=useState(false),[users,setUsers]=useState<User[]>([]),[editingUser,setEditingUser]=useState<User|null>(null),[groups,setGroups]=useState<Group[]>([]),[events,setEvents]=useState<Event[]>([]),[notes,setNotes]=useState<Notification[]>([]);
  const[permissions,setPermissions]=useState<{role:string;actions:string[];user_ids:string[];group_ids:string[]}|null>(null),[query,setQuery]=useState(''),[error,setError]=useState(''),[modal,setModal]=useState(false),[editingFolder,setEditingFolder]=useState<Folder|null>(null);
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="nav-label">ARQUIVOS</div><nav className="side-nav"><button className="nav-item" onClick={()=>setView('documents')}><Icon name="file"/><span>Documentos</span></button><button className="nav-item"><Icon name="image"/><span>Imagens</span></button><button className="nav-item"><Icon name="video"/><span>Vídeos</span></button><button className="nav-item"><Icon name="more"/><span>Outros</span></button></nav>
      <div className="nav-label">PROJETOS</div><nav className="side-nav"><button className="nav-item"><Icon name="grid"/><span>Projetos</span></button><button className="nav-item"><Icon name="folder"/><span>Arquivos do projeto</span></button></nav>
      <div className="nav-label">FERRAMENTAS</div><nav className="side-nav"><button className="nav-item"><Icon name="sign"/><span>Assinatura digital</span></button><button className="nav-item"><Icon name="scan"/><span>OCR / Digitalização</span></button><button className="nav-item"><Icon name="cloud"/><span>Backup e restauração</span></button></nav>
    ...
    <div className="main-shell">
      ...
      <main className="content">
        ...
        {(view==='documents'||view==='favorites'||view==='recent'||view==='search')&&<Section title={title}><DocumentTable docs={docs} onSelect={openDoc} onAction={action}/></Section>}
  ...
  {searchTotal>searchPageSize&&<div className="pagination"><Button variant="secondary" disabled={searchPage<=1} onClick={()=>{setSearchPage(p=>p-1);setTimeout(()=>load('advanced-search'),0)}}>Anterior</Button><span>Página {searchPage} de {Math.ceil(searchTotal/searchPageSize)}</span><Button variant="secondary" disabled={searchPage>=Math.ceil(searchTotal/searchPageSize)} onClick={()=>{setSearchPage(p=>p+1);setTimeout(()=>load('advanced-search'),0)}}>Próxima</Button></div>}
        ...
        {view==='folders'&&<Section title={showFolderTrash?'Lixeira de pastas':'Pastas'} actions={<Button variant="secondary" onClick={async()=>{setShowFolderTrash(!showFolderTrash);if(!showFolderTrash)setDeletedFolders(await api.folderTrash())}}>{showFolderTrash?'Voltar às pastas':'Lixeira de pastas'}</Button>}><div className="card-grid">{(showFolderTrash?deletedFolders:folders).map(f=><article className="mini-card" key={f.id}><span className="folder-icon">□</span><strong>{f.name}</strong><small>{f.parent_id?'Subpasta':'Pasta raiz'}</small><div>{showFolderTrash?<Button variant="secondary" onClick={async()=>{await api.restoreFolder(f.id);setDeletedFolders(await api.folderTrash())}}>Restaurar</Button>:<><Button variant="secondary" onClick={()=>{setEditingFolder(f);setModal(true)}}>Renomear</Button><Button variant="danger" onClick={async()=>{await api.deleteFolder(f.id);load('folders')}}>Excluir</Button></>}</div></article>)}</div>{!(showFolderTrash?deletedFolders:folders).length&&<StatePanel title="Nenhuma pasta" message={showFolderTrash?'A lixeira de pastas está vazia.':'Crie uma pasta para começar a organizar seus documentos.'}/>}</Section>}
        {view==='profile'&&<Section title="Minha conta"><form className="profile-form" onSubmit={async e=>{e.preventDefault();const f=new FormData(e.currentTarget);setUser(await api.updateMe({name:String(f.get('name')),phone:String(f.get('phone'))}))}}><label>Nome<input name="name" defaultValue={user.name}/></label><label>E-mail<input value={user.email} disabled/></label><label>Telefone<input name="phone" defaultValue={user.phone||''}/></label><Button>Salvar perfil</Button></form></Section>}
        {view==='users'&&<Section title="Usuários">{user.role==='admin'?<><Button onClick={()=>{setEditingUser(null);setModal(true)}}>Criar usuário</Button><div className="user-list">{users.map(u=><article key={u.id}><strong>{u.name}</strong><span>{u.email} · {u.role} · {u.status||'active'}</span><div><Button variant="secondary" onClick={()=>{setEditingUser(u);setModal(true)}}>Editar</Button><Button variant="secondary" onClick={async()=>{await api.adminUpdateUser(u.id,{status:u.status==='active'?'inactive':'active'});setUsers(await api.users())}}>{u.status==='active'?'Desativar':'Ativar'}</Button></div></article>)}</div></>:<StatePanel title="Acesso restrito" message="Somente administradores podem gerenciar usuários."/>}</Section>}
        {view==='groups'&&<Section title="Grupos"><Button onClick={async()=>{const name=window.prompt('Nome do grupo');if(name){await api.createGroup(name);setGroups(await api.groups())}}}>Criar grupo</Button><div className="card-grid">{groups.map(g=><article className="mini-card" key={g.id}><strong>{g.name}</strong><small>{g.member_ids.length} membro(s)</small>{user.role==='admin'&&<><select onChange={async e=>{if(e.target.value){await api.addMember(g.id,e.target.value);setGroups(await api.groups())}}} defaultValue=""><option value="">Adicionar usuário...</option>{users.filter(u=>!g.member_ids.includes(u.id)).map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select><div>{g.member_ids.map(id=>{const m=users.find(x=>x.id===id);return <button key={id} onClick={async()=>{await api.removeMember(g.id,id);setGroups(await api.groups())}}>{m?.name||id} ×</button>})}</div></>}</article>)}</div></Section>}
        {view==='permissions'&&selected&&permissions&&<Section title={'Permissões: '+selected.name}><p>Papel: <strong>{permissions.role}</strong></p><div className="permission-grid">{['read','write','delete','share'].map(a=><label key={a}><input type="checkbox" checked={permissions.actions.includes(a)} onChange={e=>setPermissions({...permissions,actions:e.target.checked?[...permissions.actions,a]:permissions.actions.filter(x=>x!==a)})}/> {a}</label>)}</div><label>Usuários autorizados<select multiple value={permissions.user_ids} onChange={e=>setPermissions({...permissions,user_ids:Array.from(e.target.selectedOptions,o=>o.value)})}>{users.filter(u=>u.id!==selected.owner_id).map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select></label><label>Grupos autorizados<select multiple value={permissions.group_ids} onChange={e=>setPermissions({...permissions,group_ids:Array.from(e.target.selectedOptions,o=>o.value)})}>{groups.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select></label>{user.role==='admin'&&<Button onClick={async()=>{await api.setPermissions(selected.id,permissions);setPermissions(await api.permissions(selected.id))}}>Salvar política</Button>}</Section>}
